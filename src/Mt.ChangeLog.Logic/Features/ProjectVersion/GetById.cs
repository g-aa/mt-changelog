...
public static class GetById
{
    ...
    public sealed class Handler : IRequestHandler<Query, ProjectVersionModel>
    {
        ...
        public Task<ProjectVersionModel> Handle(Query request, CancellationToken cancellationToken)
        {
            ...
            _logger.LogDebug("Получен запрос на предоставление данных о версии проекта '{Model}'.", model);

            var result = _context.ProjectVersions.AsNoTracking()
            ...
            _logger.LogDebug("Запрос на получение данных о версии проекта '{Result}' выполнен успешно.", result);
